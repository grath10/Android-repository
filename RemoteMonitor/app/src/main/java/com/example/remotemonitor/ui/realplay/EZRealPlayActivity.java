package com.example.remotemonitor.ui.realplay;
...
import com.example.remotemonitor.MonitorApplication;
import com.example.remotemonitor.ui.cameralist.EZCameraListActivity;
import com.example.remotemonitor.ui.common.ScreenOrientationHelper;
import com.example.remotemonitor.ui.util.ActivityUtils;
import com.example.remotemonitor.ui.util.AudioPlayUtil;
import com.example.remotemonitor.ui.util.DataManager;
import com.example.remotemonitor.ui.util.EZUtils;
import com.example.remotemonitor.ui.util.VerifyCodeInput;
import com.example.remotemonitor.widget.WaitDialog;
import com.example.remotemonitor.widget.loading.LoadingTextView;
    ...
    private void ptzOption(final EZPTZCommand command, final EZPTZAction action) {
        new Thread(new Runnable() {
            ...
            public void run() {
                boolean ptz_result = false;
                try {
                    ptz_result = MonitorApplication.getOpenSDK().controlPTZ(mCameraInfo.getDeviceSerial(), mCameraInfo.getCameraNo(), command,
            ...
            Thread thr = new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        MonitorApplication.getOpenSDK().setVideoLevel(mCameraInfo.getDeviceSerial(), mCameraInfo.getCameraNo(), mode.getVideoLevel());
...
//        Toast.makeText(EZRealPlayActivity.this, getResources().getString(R.string.already_saved_to_volume), Toast.LENGTH_SHORT).show();
    ...
    private void startRealPlay() {
        ...
        if (mCameraInfo != null) {
            if (mEZPlayer == null) {
                mEZPlayer = MonitorApplication.getOpenSDK().createPlayer(mCameraInfo.getDeviceSerial(), mCameraInfo.getCameraNo());
            }
            ...
            mEZPlayer = MonitorApplication.getOpenSDK().createPlayerWithUrl(mRtspUrl);
